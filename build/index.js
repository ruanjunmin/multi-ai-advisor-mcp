...
server.tool("list-available-models", "List all available models in Ollama that can be used with query-models", {}, async () => {
    try {
...
server.tool("query-models", "Query multiple AI models via Ollama and get their responses to compare perspectives", {
    ...
    try {
        ...
        const responses = await Promise.all(modelsToQuery.map(async (modelName) => {
            try {
